...
export function findPassenger(passengers, name) {
  ...
  return passengers.find( element => element.name.toLowerCase() === name.toLowerCase() );
}

export function getPassengerIndex(passengers, name) {
  if (!Array.isArray(passengers) || typeof name !== 'string') return -1;
  return passengers.findIndex( element => element.name.toLowerCase() === name.toLowerCase() );
}
...
export function getWaitlistedPassengers(passengers) {
  ...
  return passengers.filter( element => element.status === "waitlisted" );
}
