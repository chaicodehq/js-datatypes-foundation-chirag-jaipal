...
export function parseFare(fareString) {
  ...
  if (Number.isNaN(result) || typeof fareString !== 'string') return -1;
  return result;
}
...
export function findCheapestAndCostliest(...fares) {
  const validNums = fares.filter( value => typeof value === 'number')
  if (validNums.length === 0) return null;
  const cheap = Math.min(...validNums);
  const costly = Math.max(...validNums);
...
export function getDistanceDifference(from, to) {
  ...
  if (Number.isNaN(num1) || Number.isNaN(num2)) return -1;
