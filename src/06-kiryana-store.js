...
export function calculateTotal(items) {
  ...
  return items.reduce( (previousValue, currentValue) => previousValue + (currentValue.price * currentValue.qty), 0);
}

export function sortByPrice(items, ascending) {
  if (!Array.isArray(items)) return [];
  return ascending === true ? [...items].sort( (a,b) => a.price - b.price ) : [...items].sort( (a,b) => b.price - a.price );
}

export function formatBill(items) {
  if (!Array.isArray(items) || items.length === 0) return "";
  return items.map( item => item = `${item.name} x ${item.qty} = Rs.${item.price * item.qty}`).join("\n");
}
